...
activiti.task.design = function() {
    var grid1;
    return{
        init : function() {
            grid1 = new activiti.DataGrid("activiti-task-grid1",{
                postData : {
                    // staffId:_UserDetail.staffId,
                    // orgId: _UserDetail.orgId
                },
                url : CONTEXT_PATH + '/task/func/task/pagin.do',shrinkToFit:false
            });
        ...
        finish : function () {
            ...
            if (rowIds.length <= 0) {
                ...
                var url = CONTEXT_PATH + '/task/funcPage/finish.do';
                var width = null;
                var heigth = null;
        ...
        createCloseComeBack : function (param) {
            activiti.task.design.loadData();
        },
        //加载数据
        loadData : function() {
            var url = CONTEXT_PATH + '/task/func/task/pagin.do';
            var  paramObj = {};
            paramObj.page = 1;
            paramObj.limit = 10;
        ...
        viewPic : function () {
            ...
            if (rowIds.length <= 0) {
                ...
                var selItem = grid1.getSelectedItem();//所选行
                var processInstanceId = selItem.processInstanceId;
                var url = CONTEXT_PATH + '/task/funcPage/viewPic.do';
                layer.open({
                    type:1,
                    title:'流程图预览',
                    area:['800px','600px'],
                    content:'<div style="height: 550px ;width: 800px  ;overflow: scroll"><img src="'+url+'?processInstanceId='+processInstanceId+'" width="100%"/></div>'
                })
            }
        },
    }
